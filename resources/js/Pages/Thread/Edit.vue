<template>
    <div class="container mx-auto mt-6 space-y-4">
        <div>
            <h1 class="text-2xl font-bold">{{ $t('editThread') }}</h1>
        </div>

        <form v-if="user" @submit.prevent="submitForm" class="space-y-4">
            <div class="mt-6">
                {{ $t('title') }}
                <input type="text" v-model="form.title" :placeholder="$t('editThread')" required
...
export default {
    ...
    methods: {
        submitForm() {
            this.form.patch(`/threads/${this.thread.channel.slug}/${this.thread.id}`, {
                preserveScroll: true,
                onSuccess: () => {
                    this.form.reset();
                },
